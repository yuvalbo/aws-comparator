...
class IpPermission(BaseModel):
    ...
    ip_protocol: str = Field(
        ..., description="IP protocol (tcp, udp, icmp, or -1 for all)"
    )
    from_port: Optional[int] = Field(
        None, ge=-1, le=65535, description="Start of port range"
    )
    to_port: Optional[int] = Field(
        None, ge=-1, le=65535, description="End of port range"
    )
    cidr_blocks: list[str] = Field(default_factory=list, description="IPv4 CIDR blocks")
    ipv6_cidr_blocks: list[str] = Field(
        default_factory=list, description="IPv6 CIDR blocks"
    )
    source_security_groups: list[str] = Field(
        default_factory=list, description="Source security group IDs"
    )
...
class EC2Instance(AWSResource):
    ...
    vpc_id: Optional[str] = Field(
        None, pattern=r"^vpc-[a-f0-9]+$", description="VPC ID"
    )
    subnet_id: Optional[str] = Field(
        None, pattern=r"^subnet-[a-f0-9]+$", description="Subnet ID"
    )
    ...
    iam_instance_profile: Optional[str] = Field(
        None, description="IAM instance profile ARN"
    )
    ...
    architecture: Optional[str] = Field(
        None, description="Architecture (x86_64, arm64)"
    )
    root_device_type: Optional[str] = Field(
        None, description="Root device type (ebs, instance-store)"
    )
    virtualization_type: Optional[str] = Field(
        None, description="Virtualization type (hvm, paravirtual)"
    )
    monitoring_state: Optional[str] = Field(
        None, description="CloudWatch monitoring state"
    )
    tenancy: Optional[str] = Field(
        None, description="Tenancy (default, dedicated, host)"
    )
    ...
    def from_aws_response(
        cls, instance_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "EC2Instance":
        """
...
class SecurityGroup(AWSResource):
    ...
    group_id: str = Field(
        ..., pattern=r"^sg-[a-f0-9]+$", description="Security group ID"
    )
    ...
    ingress_rules: list[IpPermission] = Field(
        default_factory=list, description="Inbound rules"
    )
    ...
    @classmethod
    def from_aws_response(
        cls, sg_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "SecurityGroup":
        """
    ...
    def _parse_ip_permission(perm: dict[str, Any]) -> IpPermission:
        ...
        source_sgs = [
            sg["GroupId"] for sg in perm.get("UserIdGroupPairs", []) if "GroupId" in sg
        ]
...
class VPC(AWSResource):
    ...
    cidr_block_associations: list[dict[str, str]] = Field(
        default_factory=list, description="Additional CIDR block associations"
    )
    state: str = Field(..., description="VPC state")
    is_default: bool = Field(
        default=False, description="Whether this is the default VPC"
    )
    ...
    enable_dns_hostnames: Optional[bool] = Field(
        None, description="DNS hostnames enabled"
    )
    ...
    @classmethod
    def from_aws_response(
        cls, vpc_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "VPC":
        """
    ...
    def __str__(self) -> str:
        ...
        return (
            f"VPC(id={self.vpc_id}, cidr={self.cidr_block}, default={self.is_default})"
        )
...
class Subnet(AWSResource):
    ...
    availability_zone_id: Optional[str] = Field(
        None, description="Availability zone ID"
    )
    ...
    map_public_ip_on_launch: bool = Field(
        default=False, description="Auto-assign public IP"
    )
    assign_ipv6_address_on_creation: bool = Field(
        default=False, description="Auto-assign IPv6"
    )
    ...
    @classmethod
    def from_aws_response(
        cls, subnet_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "Subnet":
        """
...
class RouteTableRoute(BaseModel):
    ...
    destination_cidr_block: Optional[str] = Field(
        None, description="Destination IPv4 CIDR"
    )
    destination_ipv6_cidr_block: Optional[str] = Field(
        None, description="Destination IPv6 CIDR"
    )
    ...
    network_interface_id: Optional[str] = Field(
        None, description="Network interface ID"
    )
    vpc_peering_connection_id: Optional[str] = Field(
        None, description="VPC peering connection ID"
    )
    ...
    def __str__(self) -> str:
        ...
        dest = (
            self.destination_cidr_block or self.destination_ipv6_cidr_block or "unknown"
        )
        target = (
            self.gateway_id
            or self.nat_gateway_id
            or self.instance_id
            or self.network_interface_id
            or "unknown"
        )
...
class RouteTable(AWSResource):
    ...
    @classmethod
    def from_aws_response(
        cls, rt_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "RouteTable":
        """
...
class NetworkAclEntry(BaseModel):
    ...
    rule_number: int = Field(
        ..., ge=1, le=32767, description="Rule number (32767 is default deny rule)"
    )
...
class NetworkAcl(AWSResource):
    ...
    network_acl_id: str = Field(
        ..., pattern=r"^acl-[a-f0-9]+$", description="Network ACL ID"
    )
    vpc_id: str = Field(..., pattern=r"^vpc-[a-f0-9]+$", description="VPC ID")
    is_default: bool = Field(default=False, description="Is default NACL")
    entries: list[NetworkAclEntry] = Field(
        default_factory=list, description="NACL entries"
    )
    associations: list[dict[str, str]] = Field(
        default_factory=list, description="Subnet associations"
    )
    ...
    @classmethod
    def from_aws_response(
        cls, nacl_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "NetworkAcl":
        """
...
class KeyPair(AWSResource):
    ...
    @classmethod
    def from_aws_response(
        cls, key_data: dict[str, Any], tags: Optional[dict[str, str]] = None
    ) -> "KeyPair":
        """
