...
class SQSQueue(AWSResource):
    ...
    delay_seconds: int = Field(
        default=0, ge=0, le=900, description="Delivery delay in seconds"
    )
    ...
    receive_message_wait_time_seconds: int = Field(
        default=0, ge=0, le=20, description="Long polling wait time in seconds"
    )
    visibility_timeout: int = Field(
        default=30, ge=0, le=43200, description="Visibility timeout in seconds"
    )
    ...
    redrive_allow_policy: Optional[dict[str, Any]] = Field(
        None, description="Dead letter queue redrive allow policy"
    )
    ...
    kms_data_key_reuse_period_seconds: int = Field(
        default=300, description="KMS data key reuse period"
    )
    sqs_managed_sse_enabled: bool = Field(
        default=False, description="SQS-managed server-side encryption enabled"
    )

    # Metrics
    approximate_number_of_messages: int = Field(
        default=0, description="Approximate messages in queue"
    )
    approximate_number_of_messages_delayed: int = Field(
        default=0, description="Approximate delayed messages"
    )
    approximate_number_of_messages_not_visible: int = Field(
        default=0, description="Approximate messages not visible"
    )

    # Timestamps
    created_timestamp: Optional[int] = Field(
        None, description="Queue creation timestamp"
    )
    last_modified_timestamp: Optional[int] = Field(
    ...
    def from_aws_response(
        ...
        if "MessageRetentionPeriod" in attributes:
            queue_dict["message_retention_period"] = int(
        ...
        queue_dict["sqs_managed_sse_enabled"] = (
        ...
        if "RedriveAllowPolicy" in attributes:
            queue_dict["redrive_allow_policy"] = json.loads(
        ...
        if "LastModifiedTimestamp" in attributes:
            queue_dict["last_modified_timestamp"] = int(
