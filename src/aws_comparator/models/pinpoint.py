...
class PinpointApplication(AWSResource):
    ...
    creation_date: Optional[str] = Field(None, description="Application creation date ISO string")
...
class CampaignSchedule(BaseModel):
    ...
    start_time: Optional[str] = Field(None, description="Campaign start time ISO string")
    ...
    quiet_time: Optional[dict[str, Any]] = Field(None, description="Quiet time configuration")
...
class MessageConfiguration(BaseModel):
    ...
    default_message: Optional[dict[str, Any]] = Field(None, description="Default message")
    email_message: Optional[dict[str, Any]] = Field(None, description="Email message configuration")
    sms_message: Optional[dict[str, Any]] = Field(None, description="SMS message configuration")
    push_notification_message: Optional[dict[str, Any]] = Field(
        None,
        description="Push notification message configuration"
    )
    in_app_message: Optional[dict[str, Any]] = Field(None, description="In-app message configuration")
    custom_message: Optional[dict[str, Any]] = Field(None, description="Custom message configuration")
...
class PinpointCampaign(AWSResource):
    ...
    holdout_percent: Optional[int] = Field(None, ge=0, le=100, description="Holdout percentage")
    hook: Optional[dict[str, Any]] = Field(None, description="Campaign hook configuration")
    ...
    last_modified_date: Optional[str] = Field(None, description="Last modified date ISO string")
    ...
    def from_aws_response(
        ...
        campaign_dict['treatment_description'] = campaign_data.get('TreatmentDescription')
...
class SegmentDimensions(BaseModel):
    ...
    attributes: Optional[dict[str, Any]] = Field(None, description="Attribute dimensions")
    behavior: Optional[dict[str, Any]] = Field(None, description="Behavior dimensions")
    demographic: Optional[dict[str, Any]] = Field(None, description="Demographic dimensions")
    ...
    user_attributes: Optional[dict[str, Any]] = Field(None, description="User attribute dimensions")
...
class PinpointSegment(AWSResource):
    ...
    # Segment definition
    dimensions: Optional[SegmentDimensions] = Field(None, description="Segment dimensions")
    import_definition: Optional[SegmentImportDefinition] = Field(
        None,
        description="Import definition for imported segments"
    )

    # Segment groups (for complex segments)
    segment_groups: Optional[dict[str, Any]] = Field(None, description="Segment groups configuration")
...
class PinpointChannel(AWSResource):
    ...
    # Metrics
    has_credential: bool = Field(default=False, description="Whether credentials are configured")
    ...
    def from_aws_response(
        ...
        if channel_type == ChannelType.EMAIL:
            ...
            channel_dict['configuration'] = {
                ...
                'default_authentication_method': channel_data.get('DefaultAuthenticationMethod'),
            }
        else:
            # Store all configuration for other channel types
            channel_dict['configuration'] = {k: v for k, v in channel_data.items()
                                            if k not in ['ApplicationId', 'Enabled', 'IsArchived',
                                                         'Platform', 'HasCredential', 'Version',
                                                         'CreationDate', 'LastModifiedDate', 'Id']}
...
class PinpointEventStream(AWSResource):
    ...
    last_modified_date: Optional[str] = Field(None, description="Last modified date ISO string")
    last_updated_by: Optional[str] = Field(None, description="Last updated by user/service")

    @classmethod
    def from_aws_response(
        cls,
        event_stream_data: dict[str, Any],
        application_id: str
    ) -> "PinpointEventStream":
        """
