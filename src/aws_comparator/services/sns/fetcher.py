...
class SNSFetcher(BaseServiceFetcher):
    ...
    def fetch_resources(self) -> dict[str, list[AWSResource]]:
        ...
        return {
            ...
            'subscriptions': self._safe_fetch('subscriptions', self._fetch_subscriptions)
        }
        ...
        try:
            ...
            for topic_arn in topic_arns:
                try:
                    ...
                    attr_response = self.client.get_topic_attributes(
                        TopicArn=topic_arn
                    )
                    ...
                    if error_code in ['AccessDenied', 'NotFound']:
                        ...
                        self.logger.error(
                            f"Error fetching topic {topic_name}: {e}",
                            exc_info=True
                        )
