...
def test_fetch_security_groups_success() -> None:
    ...
    test_sg = next(
        (sg for sg in resources["security_groups"] if sg.group_name == "test-sg"), None
    )
...
def test_fetch_subnets_success() -> None:
    ...
    test_subnet = next(
        (s for s in resources["subnets"] if s.subnet_id == subnet_id), None
    )
...
def test_fetch_route_tables_success() -> None:
    ...
    # Tag route table
    client.create_tags(Resources=[rt_id], Tags=[{"Key": "Name", "Value": "test-rt"}])
    ...
    test_rt = next(
        (r for r in resources["route_tables"] if r.route_table_id == rt_id), None
    )
...
def test_fetch_network_acls_success() -> None:
    ...
    # Tag NACL
    client.create_tags(
        Resources=[nacl_id], Tags=[{"Key": "Name", "Value": "test-nacl"}]
    )
    ...
    # Find our test NACL
    test_nacl = next(
        (n for n in resources["network_acls"] if n.network_acl_id == nacl_id), None
    )
...
def test_fetch_security_group_with_multiple_rules() -> None:
    ...
    # Assert
    test_sg = next(
        (s for s in resources["security_groups"] if s.group_name == "multi-rule-sg"),
        None,
    )
    assert test_sg is not None
    ...
    # Verify rules
    tcp_ports = [
        rule.from_port for rule in test_sg.ingress_rules if rule.ip_protocol == "tcp"
    ]
