...
class TestEventBusFetching:
    ...
    def test_fetch_custom_event_bus(self, eventbridge_fetcher, events_client):
        ...
        # Find custom bus
        custom_bus = next(
            (bus for bus in event_buses if bus.name == "custom-bus"), None
        )
    ...
    def test_fetch_event_bus_with_tags(self, eventbridge_fetcher, events_client):
        ...
        # Find tagged bus
        tagged_bus = next(
            (bus for bus in event_buses if bus.name == "tagged-bus"), None
        )
...
class TestRuleFetching:
    ...
    def test_fetch_rule_with_targets(self, eventbridge_fetcher, events_client):
        ...
        events_client.put_targets(
            Rule=rule_name,
            Targets=[
                {"Id": "1", "Arn": target_arn, "Input": json.dumps({"key": "value"})}
            ],
        )
...
class TestPagination:
    ...
    def test_paginate_event_buses(self, eventbridge_fetcher):
        ...
        with patch.object(
            eventbridge_fetcher.client, "can_paginate"
        ) as mock_can_paginate:
            with patch.object(
                eventbridge_fetcher.client, "get_paginator"
                ...
                results = eventbridge_fetcher._paginate(
